...
const orderFromMenu = (itens) => {
  menu.consumption.push(itens);
};

const calculatePartial = (key, prices) => {
  let partial = 0;

  for (let item of menu.consumption) {
    if (Object.keys(prices[key]).includes(item)) {
      partial += prices[key][item];
    }
  }
...
const createMenu = (object) => {
  ...
  menu.pay = () => {
    ...
    for (let key of Object.keys(prices)) {
      bill += calculatePartial(key, prices);    
    }
